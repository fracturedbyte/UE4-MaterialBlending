...
#include "Materials/MaterialExpressionHeightmap.h" // FB Bulgakov - Material blending
...
//
//	FB Bulgakov - Material blending - UMaterialExpressionHeightmap
//
UMaterialExpressionHeightmap::UMaterialExpressionHeightmap(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{}

#if WITH_EDITOR
int32 UMaterialExpressionHeightmap::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	int32 CoordinateIndex = Coordinates.GetTracedInput().Expression ? Coordinates.Compile(Compiler) : Compiler->TextureCoordinate(ConstCoordinate, false, false);
	/*int32 TextureReferenceIndex = INDEX_NONE;
	int32 TextureCodeIndex = Compiler->Texture(Texture, TextureReferenceIndex);
	...
	return Compiler->TextureSample(
		...
		TextureReferenceIndex,
		false);*/
	return Compiler->GetHeightmapValue(CoordinateIndex);
}

void UMaterialExpressionHeightmap::GetCaption(TArray<FString>& OutCaptions) const
{
	OutCaptions.Add(TEXT("Heightmap"));
}

void UMaterialExpressionHeightmap::GetExpressionToolTip(TArray<FString>& OutToolTip)
{
	ConvertToMultilineToolTip(TEXT("Return heightmap value for current landscape."), 40, OutToolTip);
}
#endif // WITH_EDITOR
...
int32 UMaterialExpressionMakeMaterialAttributes::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex) 
{
	...
	case MP_BlendMaterialWeight: Ret = BlendMaterialWeight.Compile(Compiler); Expression = BlendMaterialWeight.Expression; break; // FB Bulgakov - Material blending
	};
	...
	: Super(ObjectInitializer)
{
	...
	Outputs.Add(FExpressionOutput(TEXT("BlendMaterialWeight"), 1, 1, 0, 0, 0)); // FB Bulgakov - Material blending
#endif
}
...
void UMaterialExpressionBreakMaterialAttributes::Serialize(FStructuredArchive::FRecord Record)
{
	...
	if (UnderlyingArchive.CustomVer(FRenderingObjectVersion::GUID) < FRenderingObjectVersion::FixedLegacyMaterialAttributeNodeTypes)
	{
		...
		Outputs[OutputIndex].SetMask(1, 1, 0, 0, 0); // FB Bulgakov - Material blending
	}
#endif // WITH_EDITOR
}
...
int32 UMaterialExpressionBreakMaterialAttributes::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	if (PropertyToIOIndexMap.Num() == 0)
	{
		...
		PropertyToIOIndexMap.Add(MP_BlendMaterialWeight, 25); // FB Bulgakov - Material blending
	}

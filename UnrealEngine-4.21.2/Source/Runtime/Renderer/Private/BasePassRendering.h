...
class FBasePassHeightmapParameters
{
public:

	void Bind(const FShaderParameterMap& ParameterMap)
	{
		HeightmapTextureParameter.Bind(ParameterMap, TEXT("HeightmapTextureValue"));
		HeightmapTextureSamplerParameter.Bind(ParameterMap, TEXT("HeightmapTextureValueSampler"));
		HeightmapBoundsParameter.Bind(ParameterMap, TEXT("HeightmapBounds"));
		LandscapeBoundsParameter.Bind(ParameterMap, TEXT("LandscapeBounds"));
	}

	void SetMeshVS(FRHICommandList& RHICmdList, FVertexShaderRHIParamRef VertexShaderRHI, const FPrimitiveSceneProxy* Proxy);
	void SetMeshPS(FRHICommandList& RHICmdList, FPixelShaderRHIParamRef PixelShaderRHI, const FPrimitiveSceneProxy* Proxy);

	friend FArchive& operator<<(FArchive& Ar, FBasePassHeightmapParameters& P)
	{
		Ar << P.HeightmapTextureParameter;
		Ar << P.HeightmapTextureSamplerParameter;
		Ar << P.HeightmapBoundsParameter;
		Ar << P.LandscapeBoundsParameter;
		return Ar;
	}

private:
	FShaderResourceParameter HeightmapTextureParameter;
	FShaderResourceParameter HeightmapTextureSamplerParameter;
	FShaderParameter HeightmapBoundsParameter;
	FShaderParameter LandscapeBoundsParameter;
};
// FB Bulgakov End
...
protected:
		...
		FMeshMaterialShader(Initializer)
	{
		...
		HeightmapParameters.Bind(Initializer.ParameterMap); // FB Bulgakov - Material blending
	}
...
public:
	...
	virtual bool Serialize(FArchive& Ar)
	{
		...
		Ar << HeightmapParameters; // FB Bulgakov - Material blending
...
private:
	...
	FBasePassHeightmapParameters HeightmapParameters; // FB Bulgakov - Material blending
};

...
bool UMaterialInstance::UpdateParameters()
{
	...
	if(IsTemplate(RF_ClassDefaultObject)==false)
	{
		...
		if(ParentMaterial)
		{
			...
			UMaterialInterface* BlendMaterialInterface = GetBlendMaterial();
			
			for (UMaterialInstance* BlendMI = Cast<UMaterialInstance>(BlendMaterialInterface);
				BlendMI;
				BlendMI = Cast<UMaterialInstance>(BlendMI->Parent))
			{
				// Scalar parameters
				bDirty = UpdateParameterSet<FScalarParameterValue, UMaterialExpressionScalarParameter>(BlendMI->ScalarParameterValues, ParentMaterial) || bDirty;

				// Vector parameters	
				bDirty = UpdateParameterSet<FVectorParameterValue, UMaterialExpressionVectorParameter>(BlendMI->VectorParameterValues, ParentMaterial) || bDirty;

				// Texture parameters
				bDirty = UpdateParameterSet<FTextureParameterValue, UMaterialExpressionTextureSampleParameter>(BlendMI->TextureParameterValues, ParentMaterial) || bDirty;

				// Font parameters
				bDirty = UpdateParameterSet<FFontParameterValue, UMaterialExpressionFontSampleParameter>(BlendMI->FontParameterValues, ParentMaterial) || bDirty;

				// Static switch parameters
				bDirty = UpdateParameterSet<FStaticSwitchParameter, UMaterialExpressionStaticBoolParameter>(BlendMI->StaticParameters.StaticSwitchParameters, ParentMaterial) || bDirty;

				// Static component mask parameters
				bDirty = UpdateParameterSet<FStaticComponentMaskParameter, UMaterialExpressionStaticComponentMaskParameter>(BlendMI->StaticParameters.StaticComponentMaskParameters, ParentMaterial) || bDirty;

				// Material layers parameters
				bDirty = UpdateParameterSet<FStaticMaterialLayersParameter, UMaterialExpressionMaterialAttributeLayers>(BlendMI->StaticParameters.MaterialLayersParameters, ParentMaterial) || bDirty;

				// Custom parameters
				for (const auto& CustomParameterSetUpdater : CustomParameterSetUpdaters)
				{
					bDirty |= CustomParameterSetUpdater.Execute(BlendMI->StaticParameters, ParentMaterial);
				}
			}
...
{	
	...
	for (UMaterialInstance* BlendMI = Cast<UMaterialInstance>(BlendMaterialInterface);
		...
		BlendMI = Cast<UMaterialInstance>(BlendMI->Parent))
	{
		GameThread_InitMIParameters(this, BlendMI->ScalarParameterValues);
		GameThread_InitMIParameters(this, BlendMI->VectorParameterValues);
		GameThread_InitMIParameters(this, BlendMI->TextureParameterValues);
		GameThread_InitMIParameters(this, BlendMI->FontParameterValues);
	}
...
UMaterialInterface* UMaterialInstanceDynamic::GetBlendMaterial() const
{
	return Parent ? Parent->GetBlendMaterial() : nullptr;
}
// FB Bulgakov End
...
void UMaterialInstance::AppendReferencedTextures(TArray<UTexture*>& InOutTextures) const
{
	...
	// FB Bulgakov Begin - Material blending
	UMaterialInterface* BMI = GetBlendMaterial();
	if (BMI)
	{
		BMI->AppendReferencedTextures(InOutTextures);
	}
	// FB Bulgakov End
}
...
void UMaterialInstance::UpdateOverridableBaseProperties()
{
	...
	// FB Bulgakov Begin - Material blending
	if (BasePropertyOverrides.bOverride_BlendMaterial)
	{
		BlendMaterial = BasePropertyOverrides.BlendMaterial;
	}
	else
	{
		BlendMaterial = Parent->GetBlendMaterial();
	}
	// FB Bulgakov End
...
void UMaterialInstance::GetBasePropertyOverridesHash(FSHAHash& OutHash)const
{
	...
	// FB Bulgakov Begin - Material blending
	UMaterialInterface* BlendMaterialInterface = GetBlendMaterial();
	if (BlendMaterialInterface != Mat->GetBlendMaterial())
	{
		FString BlendMaterialName = BlendMaterialInterface ? BlendMaterialInterface->GetName() : "None";

		const FString HashString = TEXT("bOverride_BlendMaterial");
		Hash.UpdateWithString(*HashString, HashString.Len());
		Hash.UpdateWithString(*BlendMaterialName, BlendMaterialName.Len());
		bHasOverrides = true;
	}
	// FB Bulgakov End
...
bool UMaterialInstance::HasOverridenBaseProperties()const
{
	...
	if (Parent && Material && Material->bUsedAsSpecialEngineMaterial == false &&
		...
		(GetBlendMaterial() != Parent->GetBlendMaterial()) // FB Bulgakov - Material blending
		))
	{
		return true;
	}
...
// FB Bulgakov Begin - Material blending
UMaterialInterface* UMaterialInstance::GetBlendMaterial() const
{

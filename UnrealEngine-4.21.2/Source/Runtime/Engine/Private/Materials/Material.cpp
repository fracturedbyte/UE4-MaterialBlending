...
int32 FMaterialResource::CompilePropertyAndSetMaterialProperty(EMaterialProperty Property, FMaterialCompiler* Compiler, EShaderFrequency OverrideShaderFrequency, bool bUsePreviousFrameTime) const
{
	...
	UMaterialInterface* BlendMaterialInterface = MaterialInterface->GetBlendMaterial();
	...
	switch(Property)
	{
		...
		case MP_BaseColor: 
		{
			...
			if (BlendMaterialInterface)
			{
				int32 BlendCode = BlendMaterialInterface->CompileProperty(Compiler, MP_BaseColor, MFCF_ForceCast);
				Ret = Compiler->Lerp(Ret, BlendCode, Compiler->BlendMaterialWeight());
			}
		}
			break;
		...
		default:
			...
			if (BlendMaterialInterface &&
				...
				Property == MP_Normal))
			{
				int32 BlendCode = BlendMaterialInterface->CompileProperty(Compiler, Property);
				if (Property == MP_Normal)
				{
					//Ret = Compiler->Div(Ret, Compiler->SquareRoot(Compiler->Dot(Ret, Ret)));
					//BlendCode = Compiler->Div(BlendCode, Compiler->SquareRoot(Compiler->Dot(BlendCode, BlendCode)));

					int32 BlendMaterial = Compiler->BlendMaterialWeight();
					int32 BlendMaterial3 = Compiler->AppendVector(Compiler->AppendVector(BlendMaterial, BlendMaterial), Compiler->Constant(0.0f));
					Ret = Compiler->Add(Ret, Compiler->Mul(BlendCode, BlendMaterial3));
				}
				else
					Ret = Compiler->Lerp(Ret, BlendCode, Compiler->BlendMaterialWeight());
			}
...
void UMaterial::Serialize(FArchive& Ar)
{
	...
	static_assert(MP_MAX == 30, "New material properties must have DoMaterialAttributesReorder called on them to ensure that any future reordering of property pins is correctly applied."); // FB Bulgakov - Material blending
...
void UMaterial::PostLoad()
{
	...
	DoMaterialAttributeReorder(&BlendMaterialWeight, UE4Ver); // FB Bulgakov - Material blending
#endif // WITH_EDITORONLY_DATA
...
FExpressionInput* UMaterial::GetExpressionInputForProperty(EMaterialProperty InProperty)
{
	switch (InProperty)
	{
		...
		case MP_BlendMaterialWeight:	return &BlendMaterialWeight; // FB Bulgakov - Material blending
	}
...
int32 UMaterial::CompilePropertyEx( FMaterialCompiler* Compiler, const FGuid& AttributeID )
{
	...
	switch (Property)
	{
		...
		case MP_BlendMaterialWeight:	return BlendMaterialWeight.CompileWithDefault(Compiler, Property); 
...
// FB Bulgakov Begin - Material blending
UMaterialInterface* UMaterial::GetBlendMaterial() const
{
...
bool UMaterial::IsPropertyActive(EMaterialProperty InProperty) const 
{
	...
	case MP_PixelDepthOffset:
		...
		break;
	// FB Bulgakov Begin - Material blending
	case MP_BlendMaterialWeight:
		Active = BlendMaterial != nullptr;
		break;
	// FB Bulgakov End
	case MP_MaterialAttributes:
	default:

...
#endif
{
	...
	// FB Bulgakov Begin - Material blending
	for (TActorIterator<ALandscape> it(InComponent->GetWorld()); it; ++it)
	{
		ALandscape* Landscape = *it;
		USceneComponent* RootComponent = Landscape->GetRootComponent();
		if (RootComponent)
		{
			int NumChild = RootComponent->GetNumChildrenComponents();
	
			//TMap<UTexture*, TArray<FVector4>> Maps;
			//TArray<UTexture*> OutTextures;
			//for (int i = 0; i < NumChild; ++i)
			//{
			//	ULandscapeComponent* pLandscapeComponent = Cast<ULandscapeComponent>(RootComponent->GetChildComponent(i));
			//	if (pLandscapeComponent)
			//	{
			//		Maps.FindOrAdd(pLandscapeComponent->HeightmapTexture).Add(pLandscapeComponent->HeightmapScaleBias);
			//		OutTextures.AddUnique(pLandscapeComponent->HeightmapTexture);
			//	}
			//}
			...
			{
				ULandscapeComponent* pLandscapeComponent = Cast<ULandscapeComponent>(RootComponent->GetChildComponent(i));
				if (pLandscapeComponent && pLandscapeComponent->Bounds.GetBox().IntersectXY(InComponent->Bounds.GetBox()))
				{
					FBox BBox = pLandscapeComponent->Bounds.GetBox();
					FVector2D BoundMax = FVector2D(BBox.Max.X, BBox.Max.Y);
					FVector2D BoundMin = FVector2D(BBox.Min.X, BBox.Min.Y);
					
					FVector2D HeightmapScale = FVector2D(pLandscapeComponent->HeightmapScaleBias.X / pLandscapeComponent->WeightmapScaleBias.X, pLandscapeComponent->HeightmapScaleBias.Y / pLandscapeComponent->WeightmapScaleBias.Y);
					FVector2D HeightmapBias = FVector2D(pLandscapeComponent->HeightmapScaleBias.Z, pLandscapeComponent->HeightmapScaleBias.W);
	
					// FinalUV = (UV - BoundMin) / (BoundMax - BoundMin) * Scale + Bias
					// = UV * A + B
					// A = Scale / (BoundMax - BoundMin)
					// B = Bias - A * BoundMin
	
					FVector2D C = FVector2D(1.0f, 1.0f) / (BoundMax - BoundMin);
					FVector2D D = -BoundMin * C;
	
					FVector2D A = HeightmapScale / (BoundMax - BoundMin);
					FVector2D B = HeightmapBias - A * BoundMin;
	
	
					//if (pLandscapeComponent->MaterialInstances.IsValidIndex(0))
					//{
					//	UMaterialInstanceConstant* LandscapeMaterialInstance = pLandscapeComponent->MaterialInstances[0];
					//	for (int i = 0; i < InComponent->GetNumMaterials(); ++i)
					//	{
					//		UMaterialInterface* Material = InComponent->GetMaterial(i);
					//		if (Material)
					//		{
					//			UMaterialInstanceConstant* MaterialInstance = NewObject<UMaterialInstanceConstant>();
					//			MaterialInstance->Parent = Material->GetMaterial();
					//			MaterialInstance->VectorParameterValues = LandscapeMaterialInstance->VectorParameterValues;
					//			MaterialInstance->TextureParameterValues = LandscapeMaterialInstance->TextureParameterValues;
					//			//LandscapeMaterialInstance->Material
					//
					//
					//			const_cast<UPrimitiveComponent*>(InComponent)->SetMaterial(i, MaterialInstance);
					//		}
					//	}
					//}
	
					HeightmapTextureData = pLandscapeComponent->HeightmapTexture;
					HeightmapBoundsData = FVector4(A.X, A.Y, B.X, B.Y);
					ComponentBoundsData = FVector4(C.X, C.Y, D.X, D.Y);
					break;
				}
			}
		}

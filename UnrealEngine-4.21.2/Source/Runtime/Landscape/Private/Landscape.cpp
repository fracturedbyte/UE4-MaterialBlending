...
void LandscapeMaterialsParameterValuesGetter(FStaticParameterSet& OutStaticParameterSet, UMaterialInstance* Material)
{
	if (Material->Parent)
	{
		...
		if (Material->GetBlendMaterial() &&
			Material->GetBlendMaterial()->GetMaterial())
		{
			UMaterial* BlendMaterial = Material->GetBlendMaterial()->GetMaterial();
			BlendMaterial->GetAllParameterInfo<UMaterialExpressionLandscapeLayerWeight>(OutParameterInfo, Guids);
			BlendMaterial->GetAllParameterInfo<UMaterialExpressionLandscapeLayerSwitch>(OutParameterInfo, Guids);
			BlendMaterial->GetAllParameterInfo<UMaterialExpressionLandscapeLayerSample>(OutParameterInfo, Guids);
			BlendMaterial->GetAllParameterInfo<UMaterialExpressionLandscapeLayerBlend>(OutParameterInfo, Guids);
			BlendMaterial->GetAllParameterInfo<UMaterialExpressionLandscapeVisibilityMask>(OutParameterInfo, Guids);
			...
			for (int32 ParameterIdx = 0; ParameterIdx < OutParameterInfo.Num(); ParameterIdx++)
			{
				...
				ParentParameter.ParameterInfo = ParameterInfo;
				// Get the settings from the parent in the MIC chain
				Material->GetBlendMaterial()->GetTerrainLayerWeightParameterValue(ParameterInfo, ParentParameter.WeightmapIndex, ExpressionId);
				ParentParameter.ExpressionGUID = ExpressionId;

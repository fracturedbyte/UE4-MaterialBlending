		...
		for (TActorIterator<ALandscape> it(GetWorld()); it; ++it)
		{
			...
			if (RootComponent)
			{
				...
				for (int i = 0; i < NumChild; ++i)
				{
					...
					if (pLandscapeComponent && pLandscapeComponent->Bounds.GetBox().IntersectXY(Bounds.GetBox()))
					{
						...
						if (pLandscapeComponent->MaterialInstances.IsValidIndex(0))
						{
							...
							for (int j = 0; j < GetNumMaterials(); ++j)
							{
								UMaterialInterface* Material = GetMaterial(j);
								if (!Material)
									continue;
								...
								// Scenario 1
								if (!MI || !MI->BasePropertyOverrides.bLandscapeBlendMaterial)
								{
									UMaterialInstanceConstant* NewBlendMaterial = NewObject<UMaterialInstanceConstant>(LandscapeMaterialInstance->GetOuter());
									NewBlendMaterial->BasePropertyOverrides.bOverride_BlendMaterial = true;
									NewBlendMaterial->BasePropertyOverrides.bLandscapeBlendMaterial = true;
									NewBlendMaterial->BasePropertyOverrides.BlendMaterial = LandscapeMaterialInstance;
									NewBlendMaterial->SetParentEditorOnly(Material);
									SetMaterial(j, NewBlendMaterial);
									//NewBlendMaterial->SetFeatureLevelToCompile()
									NewBlendMaterial->MarkPackageDirty();
									MarkRenderStateDirty();
									NewBlendMaterial->ForceRecompileForRendering();
									// Force to recompile
								}
								// Scenario 2
								else if (MI->BasePropertyOverrides.BlendMaterial != LandscapeMaterialInstance)
								{

									MI->BasePropertyOverrides.BlendMaterial = LandscapeMaterialInstance;
									MI->RecacheUniformExpressions();
									// Force update MI
								}
								// Scenario 3
								else

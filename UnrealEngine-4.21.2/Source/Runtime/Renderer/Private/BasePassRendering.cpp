...
void FBasePassHeightmapParameters::SetMeshVS(FRHICommandList& RHICmdList, FVertexShaderRHIParamRef VertexShaderRHI, const FPrimitiveSceneProxy* Proxy)
{
	if (Proxy)
	{
		if (HeightmapTextureParameter.IsBound() && HeightmapTextureSamplerParameter.IsBound())
		{
			FTextureRHIParamRef HeightmapTexture = GSystemTextures.PerlinNoiseGradient->GetRenderTargetItem().ShaderResourceTexture;

			if (Proxy->HeightmapTextureData && Proxy->HeightmapTextureData->Resource)
			{
				HeightmapTexture = Proxy->HeightmapTextureData->Resource->TextureRHI;
			}

			SetTextureParameter(RHICmdList, VertexShaderRHI, HeightmapTextureParameter, HeightmapTextureSamplerParameter, TStaticSamplerState<SF_Trilinear, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI(), HeightmapTexture);
		}
		
		if (HeightmapBoundsParameter.IsBound())
		{
			SetShaderValue(RHICmdList, VertexShaderRHI, HeightmapBoundsParameter, Proxy->HeightmapBoundsData);
		}

		if (LandscapeBoundsParameter.IsBound())
		{
			SetShaderValue(RHICmdList, VertexShaderRHI, LandscapeBoundsParameter, Proxy->ComponentBoundsData);
		}
	}
}

void FBasePassHeightmapParameters::SetMeshPS(FRHICommandList& RHICmdList, FPixelShaderRHIParamRef PixelShaderRHI, const FPrimitiveSceneProxy* Proxy)
{
	if (Proxy)
	{
		if (HeightmapTextureParameter.IsBound() && HeightmapTextureSamplerParameter.IsBound())
		{
			...
			SetTextureParameter(RHICmdList, PixelShaderRHI, HeightmapTextureParameter, HeightmapTextureSamplerParameter, TStaticSamplerState<SF_Trilinear, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI(), HeightmapTexture);
		}

		if (HeightmapBoundsParameter.IsBound())
		{
			SetShaderValue(RHICmdList, PixelShaderRHI, HeightmapBoundsParameter, Proxy->HeightmapBoundsData);
		}

		if (LandscapeBoundsParameter.IsBound())
		{
			SetShaderValue(RHICmdList, PixelShaderRHI, LandscapeBoundsParameter, Proxy->ComponentBoundsData);
		}
	}
}
// FB Bulgakov End

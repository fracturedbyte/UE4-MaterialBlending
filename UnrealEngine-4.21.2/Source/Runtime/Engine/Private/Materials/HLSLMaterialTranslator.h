...
public: 
	...
	,	DynamicParticleParameterMask(0)
	{
		...
		SharedPixelProperties[MP_BlendMaterialWeight] = true; // FB Bulgakov - Material blending
	...
	bool Translate()
	{
		STAT(double HLSLTranslateTime = 0);
		{
			...
			int32 BlendMaterialWeightCodeChunkEnd = -1; // FB Bulgakov - Material blending
			...
			const EShaderFrequency BlendMaterialWeightShaderFrequency = FMaterialAttributeDefinitionMap::GetShaderFrequency(MP_BlendMaterialWeight); // FB Bulgakov - Material blending
			...
			// FB Bulgakov Begin - Material blending
			{
				check(SharedPropertyCodeChunks[BlendMaterialWeightShaderFrequency].Num() == 0);
				Chunk[MP_BlendMaterialWeight] = Material->CompilePropertyAndSetMaterialProperty(MP_BlendMaterialWeight, this);
				BlendMaterialWeightCodeChunkEnd = SharedPropertyCodeChunks[BlendMaterialWeightShaderFrequency].Num();
			}
			// FB Bulgakov End
			...
			// FB Bulgakov Begin - Material blending
			{
				GetFixedParameterCode(
					0,
					BlendMaterialWeightCodeChunkEnd,
					Chunk[MP_BlendMaterialWeight],
					SharedPropertyCodeChunks[BlendMaterialWeightShaderFrequency],
					TranslatedCodeChunkDefinitions[MP_BlendMaterialWeight],
					TranslatedCodeChunks[MP_BlendMaterialWeight]);

				if (TranslatedCodeChunkDefinitions[MP_BlendMaterialWeight].IsEmpty())
				{
					TranslatedCodeChunkDefinitions[MP_BlendMaterialWeight] = GetDefinitions(SharedPropertyCodeChunks[BlendMaterialWeightShaderFrequency], 0, BlendMaterialWeightCodeChunkEnd);
				}
			}
			// FB Bulgakov End

			// Do Normal Chunk after blending
			{
				int32 StartChunk = 0;
				if (BlendMaterialWeightShaderFrequency == NormalShaderFrequency && SharedPixelProperties[MP_Normal])
				{
					// When processing shared properties, do not generate the code before the Normal was generated as those are already handled
					StartChunk = BlendMaterialWeightCodeChunkEnd;
				}
			...
			for(uint32 PropertyId = 0; PropertyId < MP_MAX; ++PropertyId)
			{
				if (PropertyId == MP_MaterialAttributes ||
					PropertyId == MP_Normal ||
					PropertyId == MP_BlendMaterialWeight ||
					PropertyId == MP_CustomOutput)// FB Bulgakov - Material blending
				{
					continue;
				}
				...
				// FB Bulgakov Begin - Material blending
				if (PropertyShaderFrequency == BlendMaterialWeightShaderFrequency && SharedPixelProperties[PropertyId])
				{
					StartChunk = BlendMaterialWeightCodeChunkEnd;
				}
				// FB Bulgakov End
	...
	void GetSharedInputsMaterialCode(FString& PixelMembersDeclaration, FString& NormalAssignment, FString& BlendMaterialWeightAssignment, FString& PixelMembersInitializationEpilog) // FB Bulgakov - Material blending
	{
		{
			...
			FString BlendMaterialWeightInitializerValue; // FB Bulgakov - Material blending
			...
			for (int32 PropertyIndex = 0; PropertyIndex < MP_MAX; ++PropertyIndex)
			{
				...
				// FB Bulgakov Begin - Material blending
				else if (Property == MP_BlendMaterialWeight)
				{
					BlendMaterialWeightInitializerValue = FString::Printf(TEXT("\tPixelMaterialInputs.%s = %s;\n"), *PropertyName, *TranslatedCodeChunks[Property]);
				}
				// FB Bulgakov End
				else
				{
			...
			BlendMaterialWeightAssignment = BlendMaterialWeightInitializerValue; // FB Bulgakov - Material blending
			if (LastProperty != -1)
			{
	...
	{	
		...
		FString BlendMaterialWeightAssignment; // FB Bulgakov - Material blending

		// Stores the code to initialize all inputs after MP_Normal
		FString PixelMembersSetupAndAssignments;

		GetSharedInputsMaterialCode(PixelMembersDeclaration, NormalAssignment, BlendMaterialWeightAssignment, PixelMembersSetupAndAssignments); // FB Bulgakov - Material blending
		...
		// FB Bulgakov Begin - Material blending
		LazyPrintf.PushParam(*TranslatedCodeChunkDefinitions[MP_BlendMaterialWeight]);
		LazyPrintf.PushParam(*BlendMaterialWeightAssignment);
		// FB Bulgakov End
...
protected:
	...
	*/	
	...
	// FB Bulgakov Begin - Material blending
	virtual int32 GetHeightmapValue(int32 CoordinateIndex) override
	{
		AddEstimatedTextureSample();

		FString	GetHeightmapCode(TEXT("GetHeightmapValuePS(%s)"));
		// add the code string
		return AddCodeChunk(MCT_Float3, *GetHeightmapCode, *GetParameterCode(CoordinateIndex));
	}
	// FB Bulgakov End
	...
	// FB Bulgakov Begin - Material blending
	virtual int32 BlendMaterialWeight() override
	{
		if (ShaderFrequency != SF_Pixel && ShaderFrequency != SF_Compute)
		{
			return NonPixelShaderExpressionError();
		...
		return AddInlinedCodeChunk(MCT_Float1, TEXT("PixelMaterialInputs.BlendMaterialWeight"));
	}
	// FB Bulgakov End
